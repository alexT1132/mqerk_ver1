...
import { analyzeQuizPerformance, getAiUsage, buildFallbackAnalysis } from '../../service/quizAnalysisService';
...
import MisSolicitudesModal from './MisSolicitudesModal.jsx'; // Modal para ver solicitudes del estudiante
import {
    Upload,
    Eye,
    CheckCircle,
    ...
    CheckCircle2,
    AlertTriangle,
    AlertCircle,  // BACKEND: Icono para solicitudes
    Play,
...
export function Actividades_Alumno_comp() {
        ...
        try {
            ...
            if (wantsHistorial && quizIdParam && level !== 'buttons') {
                ...
                if (Number.isFinite(qid)) {
                    ...
                    // Limpiar URL inmediatamente para evitar reapertura al refrescar
                    params.delete('historial');
                    params.delete('quizId');
                    const finalQs = params.toString();
                    navigate(`${location.pathname}${finalQs ? `?${finalQs}` : ''}`, { replace: true });
                }
            }
            // Eliminamos el else para evitar que al limpiar la URL se pierda la bandera pendiente
        } catch (e) {
    ...
    // Modal de solicitudes del estudiante
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            if (!selectedType || !selectedArea) {
                // Evitar ejecución prematura o sin filtros
                return;
            }
    ...
    const handleVerAnalisis = async (quizId) => {
        ...
        try {
            ...
            // VERIFICACIÓN DE CUOTA DE IA
            let result = '';
            const usage = await getAiUsage(estudianteId, 'quiz');

            if (usage && usage.remaining <= 0) {
                showNotification('Límite de IA alcanzado', 'Has alcanzado tu límite diario. Se muestra un reporte basado en datos locales.', 'info');
                // Generar análisis local (fallback)
                result = buildFallbackAnalysis({ ...metaPayload, purpose: 'quiz' });
                // Marcar origen como fallback para que la UI lo sepa (aunque el texto ya trae la marca)
            } else {
                // Generar análisis con IA (si hay cuota)
                result = await analyzeQuizPerformance({ ...metaPayload, purpose: 'quiz' });
            }
                            ...
                            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <button
                                        ...
                                        className="flex items-center gap-2 px-3 py-2 text-xs sm:text-sm text-violet-700 bg-violet-50 hover:bg-violet-100 border border-violet-200 rounded-lg font-medium transition-all hover:shadow-md active:scale-95"
    ...
    return (
        <div className="min-h-screen bg-white">
            ...
            {currentLevel === 'table' && (selectedType === 'actividades' ? (
                <ActivitiesTable
                    ...
                    openLongText={openLongText}
                    openResourcesModal={openResourcesModal}
                    handleDownload={handleDownload}
                    openUploadModal={openUploadModal}
                    openViewModal={openViewModal}
                    openNotasModal={openNotasModal}
                />
            ) : (
                <QuizTable
                    selectedArea={selectedArea}
                    ...
                    isWithinDeadline={isWithinDeadline}
                    openLongText={openLongText}
                    computeQuizEstado={computeQuizEstado}
                    isQuizAvailable={isQuizAvailable}
                    getTotalAttempts={getTotalAttempts}
                    isQuizOpenElsewhere={isQuizOpenElsewhere}
