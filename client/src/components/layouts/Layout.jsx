...
export function Layout({ children, HeaderComponent, SideBarDesktopComponent, SideBarSmComponent, backgroundClassName = 'bg-white', contentClassName = '' }) {
  ...
  return (
  <div className={`min-h-screen ${backgroundClassName} relative`}>
      {/* Header principal */}
      {HeaderComponent && (
        <HeaderComponent />
      )}
  ...
  {hasSidebar && (!isCoursesRoute) && (
      ...
      <main
        className={
          `relative z-10 pt-14 pb-6 px-2 sm:px-3 md:px-4 lg:px-6 xl:px-8 overflow-x-hidden ` +
          (hasSidebar
