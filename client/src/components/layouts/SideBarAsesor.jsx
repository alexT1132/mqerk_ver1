...
// Asegúrate de importar tus contextos y api correctamente
...
/**
 * Componente ElementoSideBar
 * Ahora soporta el estilo "Relieve" y lógica de logout.
 */
function ElementoSideBar({ Icono, NombreElemento, to, onClick: mobileOnClick }) {
  ...
  // Detectar si es uso móvil (aunque este componente SideBar parece ser solo desktop por el "hidden sm:flex")
  ...
  const handleLinkClick = async (e) => {
    ...
    if (to === "/login") {
      ...
      try {
        ...
        // Limpia lo que necesites del storage
        localStorage.clear(); 
      } catch (error) {
    ...
    // Evitar recarga si ya estás en la ruta
  ...
  // --- ESTILOS ---
  let containerClasses = "group flex justify-center items-center relative z-10 min-h-[60px] w-full px-2";
  let linkClasses = "flex items-center justify-center p-0 rounded-xl w-12 h-12 aspect-square transition-all duration-300 ease-out relative select-none ";

  if (isActive) {
    // Activo PC: Gradiente, Scale, Sombra fuerte
  ...
  return (
    <li className={containerClasses} style={{ listStyle: 'none' }}>
      ...
      >
        <div className="flex-shrink-0 relative flex items-center justify-center w-full h-full">
          {/* Clonamos el icono para inyectar el color correcto */}
          {React.cloneElement(Icono, {
            fill: currentIconColor, // Tus SVGs usan fill
            className: `transition-all duration-300 ${isActive ? "w-6 h-6" : "w-5 h-5"}`,
            // Eliminamos width/height duros del SVG original para controlarlos con clases
            width: undefined,
            height: undefined,
          })}
        </div>
...
export function SideBar({ estudiante, asesor, admin }) {
  ...
  // Definición de Rutas (Mantenida intacta)
  ...
  if (roles.length > 1) {
    return (
      <div className="p-4 text-red-600 font-bold">
        Error: Solo se debe pasar una propiedad de rol (estudiante, asesor o admin).
      </div>
    );
  }

  // Iconos (Definición SVG limpia para ser controlada por el hijo)
  const xmlns = "http://www.w3.org/2000/svg";
  const viewBox = "0 -960 960 960";
  
  // Nota: Eliminé 'fill={svgColor}' hardcoded aquí para que ElementoSideBar lo controle dinámicamente
  ...
  return (
    <>
      <aside
        className="hidden sm:flex flex-col w-[80px] h-full bg-white/95 backdrop-blur-sm shadow-[4px_0_10px_-2px_rgba(0,0,0,0.1)] z-1 border-r border-gray-100"
        aria-label="Sidebar"
      >
        {/* Agregado overflow-y-auto y ocultamiento de scrollbar para evitar problemas en pantallas pequeñas */}
        <nav className="h-full overflow-y-auto overflow-x-hidden" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>

          <ul className="flex flex-col min-h-full p-2 pb-4">
            {/* GRUPO SUPERIOR */}
            <div className="space-y-1">
              ...
              {/* Lógica condicional para items específicos de cada rol */}
              {rutas.toActividades && <ElementoSideBar to={rutas.toActividades} Icono={LogoActividades} NombreElemento='Actividades' />}
              {rutas.toQuizt && <ElementoSideBar to={rutas.toQuizt} Icono={LogoQuizt} NombreElemento='Quizt' />}

              {/* Si existe la ruta (sea estudiante, admin o asesor), se renderiza */}
              {rutas.toMisCursos && <ElementoSideBar to={rutas.toMisCursos} Icono={LogoCursos} NombreElemento='Mis Cursos' />}
              {rutas.toSimuladores && <ElementoSideBar to={rutas.toSimuladores} Icono={LogoSimuladores} NombreElemento='Simuladores' />}
              {rutas.toAsesorias && <ElementoSideBar to={rutas.toAsesorias} Icono={LogoAsesorias} NombreElemento='Asesorías' />}
              ...
              {rutas.toFeedback && <ElementoSideBar to={rutas.toFeedback} Icono={LogoFeedback} NombreElemento='Feedback' />}
              {rutas.toMisPagos && <ElementoSideBar to={rutas.toMisPagos} Icono={LogoPagos} NombreElemento='Mis pagos' />}
              {rutas.toAutenticacion && <ElementoSideBar to={rutas.toAutenticacion} Icono={LogoAutenticacion} NombreElemento='Autenticación' />}
            </div>

            {/* EMPUJADOR HACIA ABAJO (FLEX GROW) - Reemplaza absolute bottom-0 */}
            <div className="flex-grow"></div>

            {/* GRUPO INFERIOR (Config y Salir) */}
