    ...
    <>
      ...
      <header className="fixed top-0 left-0 right-0 flex items-center justify-between z-50 bg-gradient-to-r from-[#3d18c3] to-[#4816bf] w-full px-4 sm:px-6 py-4 shadow-md">
        ...
        <div className="flex items-center justify-end gap-3 sm:gap-4 h-full w-fit z-10">
          ...
          <>
            {/* Icono de Chat - Réplica Exacta del Flotante (Solo visible en versión móvil) */}
            <button
              className="flex sm:hidden items-center justify-center w-11 h-11 text-white bg-violet-600 hover:bg-violet-700 rounded-full transition-all duration-300 relative group mr-2 shadow-lg hover:shadow-xl hover:scale-105 active:scale-95"
              onClick={() => window.dispatchEvent(new CustomEvent('open-chat'))}
              aria-label="Abrir chat"
            >
              {/* Efectos de ping idénticos al flotante */}
              <span className="absolute inset-0 rounded-full bg-violet-400 opacity-75 animate-ping group-hover:opacity-100" style={{ animationDuration: '2s' }}></span>
              <span className="absolute inset-0 rounded-full bg-violet-400 opacity-50 animate-ping group-hover:opacity-75" style={{ animationDuration: '2.5s', animationDelay: '0.5s' }}></span>

              {/* SVG exacto de ChatIcon - Tamaño 28px (w-7 h-7) */}
              <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="relative z-10 w-7 h-7">
            ...
            <div className="relative flex items-center justify-center">
              <div className="flex items-center justify-center w-9 h-9 sm:w-11 sm:h-11 lg:w-12 lg:h-12 rounded-full overflow-hidden border-1 sm:border-2 border-white shadow-md sm:shadow-lg hover:scale-105 transition-transform duration-200 cursor-pointer">
                {(() => {
                  const finalPhoto = buildStaticUrl(alumno?.foto);
