...
export default function LoginResponsive() {
    ...
    useEffect(() => {
      ...
      if (phase === "typing") {
        if (idx < text.length) {
          ...
          t = setTimeout(
            () => setIdx(idx + 1),
            isPause ? typeSpeed * 1.9 : typeSpeed
          );
        } else {
  ...
  return (
    <div className="relative min-h-[calc(100dvh-0px)] w-full bg-gradient-to-br from-blue-600 via-indigo-600 to-purple-700 overflow-hidden">
      ...
      {/* Contenido centrado (ajustado más arriba) */}
      <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 min-h-dvh flex flex-col items-center justify-start gap-1 sm:gap-2 py-2 sm:py-3 pt-2 sm:pt-3 lg:pt-4">
        {/* Título principal con efecto de escritura */}
        <h1
          className="text-center text-white font-black tracking-tight leading-tight mb-1 sm:mb-2 drop-shadow-[0_6px_18px_rgba(0,0,0,0.55)]
          text-[clamp(1.1rem,5.2vw,2.2rem)] sm:text-[clamp(1.7rem,5.5vw,4.2rem)] lg:text-[clamp(2.2rem,4.5vw,4.8rem)]
          whitespace-normal break-words min-h-[2.1rem] sm:min-h-[3.2rem] lg:min-h-[3.8rem]"
          aria-label={title}
          style={{
            textShadow:
        ...
        >
          ...
          <span className="ml-1 inline-block w-[1ch] align-baseline text-white/80 animate-pulse">
            |
          </span>
        </h1>

        {/* Tarjeta del formulario (glassmorphism) */}
  <div className="relative mx-auto w-full max-w-[18rem] xs:max-w-[20rem] sm:max-w-[22rem] md:max-w-[24rem] mt-8 sm:mt-12 lg:mt-16 xl:mt-20">
          {/* Halo para separar la tarjeta del fondo */}
          <div
            aria-hidden
            className="pointer-events-none absolute -inset-x-16 -top-14 h-40 rounded-full bg-white/30 blur-3xl opacity-80"
          ></div>

          <div className="relative rounded-3xl p-4 sm:p-6 backdrop-blur-2xl backdrop-saturate-150 bg-white/35 border border-white/50 shadow-[inset_0_1px_10px_rgba(255,255,255,0.22),_0_28px_80px_rgba(0,0,0,0.40)] ring-1 ring-white/70">
            ...
            {Array.isArray(errors) && errors.length > 0 && (
              <div className="mb-4 space-y-2">
                {errors.map((error, i) => (
                  <div
                    key={i}
                    className="rounded-lg bg-red-600/20 text-red-100 border border-red-400/30 px-3 py-2 text-sm"
                  >
                    {error}
                  </div>
                ))}
            ...
            <form onSubmit={onSubmit} className="space-y-5">
                ...
                <div className="mt-2 relative">
                  ...
                  >
                    {showPwd ? (
                      ...
                      >
                        <path d="M17.94 17.94A10.94 10.94 0 0 1 12 20c-5 0-9.27-3-11-8 1.04-2.71 2.98-4.94 5.41-6.31" />
                        <path d="M1 1l22 22" />
                        <path d="M9.88 9.88a3 3 0 1 0 4.24 4.24" />
                        <path d="M6.1 6.1A10.94 10.94 0 0 1 12 4c5 0 9.27 3 11 8-.64 1.67-1.64 3.16-2.87 4.35" />
                      </svg>
                    ) : (
                      // eye icon
                      <svg
              ...
              <div className="flex items-center justify-between gap-3">
                <label
                  htmlFor="remember-me"
                  className="inline-flex items-center gap-2 text-sm text-white/90 select-none"
                >
                  <input
                    id="remember-me"
                    type="checkbox"
