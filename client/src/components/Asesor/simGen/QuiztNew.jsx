...
export default function EspanolFormBuilder() {
  ...
  const handlePublish = async () => {
    ...
    try{
      ...
      const body = {
        titulo: draft?.titulo || draft?.nombre || `Quizt ${new Date().toLocaleDateString()}`,
        descripcion: draft?.instrucciones || '',
        materia: areaTitle,
        // Mapear el título del área al id_area para guardar la relación
        id_area: areaIdFromName?.(areaTitle) || null,
        publico: true,
      ...
      alert(e?.response?.data?.message || 'No se pudo crear el quiz');
    }
  };
