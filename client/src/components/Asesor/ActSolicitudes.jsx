...
const Chip = ({ active, onClick, icon, label, count }) => (
  <button
    onClick={onClick}
    className={`inline-flex items-center gap-2 rounded-md px-3.5 py-1.5 text-xs transition select-none border
      ${active
        ? "bg-slate-50 text-slate-800 border-slate-300 shadow-sm font-bold"
        : "bg-white text-slate-500 border-slate-100 hover:bg-slate-50"}
    `}
  >
    <span className={`inline-flex h-5 w-5 items-center justify-center rounded-sm transition-colors
      ${active ? "text-slate-700" : "text-slate-400"}
    `}>
      {icon}
    </span>
    <span>{label}</span>
    <span className={`rounded-md px-1.5 py-0.5 text-[10px] font-bold border transition-colors
      ${active ? "bg-white border-slate-300 text-slate-700" : "bg-slate-50 border-slate-100 text-slate-400"}
    `}>
      {count}
    </span>
  </button>
...
export default function TablaComprobantesEEAU({ initial = DEMO, titulo = "Solicitudes" }) {
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-3 py-6">
      {/* Chips */}
      <div className="mb-6 flex flex-wrap items-center gap-1.5 rounded-xl border border-slate-200 bg-white p-2 shadow-sm">
      ...
      <div className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        <div className="px-4 py-3 border-b border-slate-100 bg-slate-50/50">
          <h2 className="text-sm font-bold text-slate-700 uppercase tracking-wider">{titulo} — {filter === 'pendiente' ? 'Pendientes' : filter === 'aprobado' ? 'Aprobados' : filter === 'rechazado' ? 'Rechazados' : 'Todas'}</h2>
        </div>
          ...
          <table className="min-w-full">
            <thead className="bg-gradient-to-r from-violet-600 via-indigo-600 to-purple-600 text-white shadow-md">
              <tr className="text-left">
                <th className="w-48 px-4 py-3 text-[10px] font-bold uppercase tracking-wider border-r border-white/30">Folio</th>
                <th className="px-4 py-3 text-[10px] font-bold uppercase tracking-wider border-r border-white/30">Nombre del alumno</th>
                <th className="w-56 px-4 py-3 text-[10px] font-bold uppercase tracking-wider text-right">Acciones</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 bg-white">
              {data.map((i) => (
                <tr key={i.folio} className="hover:bg-slate-50/50 transition-colors duration-150">
                  <td className="px-4 py-3 text-[11px] font-bold text-slate-500">#{i.folio}</td>
                  <td className="px-4 py-3 text-sm font-bold text-slate-800">{i.nombre}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-2">
                      <button
                        onClick={() => setEstado(i.folio, "aprobado")}
                        className="rounded-md border border-slate-100 bg-white px-3 py-1 text-[10px] font-bold uppercase text-emerald-600 hover:bg-emerald-50 hover:shadow-sm transition-all duration-200 hover:scale-105 active:scale-95"
                      >
                        Aceptar
                      </button>
                      <button
                        onClick={() => setEstado(i.folio, "rechazado")}
                        className="rounded-md border border-slate-100 bg-white px-3 py-1 text-[10px] font-bold uppercase text-rose-600 hover:bg-rose-50 hover:shadow-sm transition-all duration-200 hover:scale-105 active:scale-95"
                      >
                        Rechazar
                      </button>
                    </div>
