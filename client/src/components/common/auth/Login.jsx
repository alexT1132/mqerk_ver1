...
export default function LoginResponsive() {
  ...
  return (
    <div className="relative min-h-[100svh] md:min-h-[100dvh] w-full bg-gradient-to-br from-blue-900 via-indigo-700 to-purple-700 overflow-hidden">
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 2xl:max-w-screen-2xl min-h-[100svh] md:min-h-[100dvh] flex flex-col items-center justify-center gap-4 sm:gap-6 2xl:gap-10 py-8">
        <h1 className="-mt-10 sm:-mt-12 2xl:-mt-16 text-center text-white/95 dark:text-white text-[clamp(1rem,3.5vw,1.6rem)] 2xl:text-4xl font-semibold tracking-tight" aria-live="polite" aria-atomic="true">
          {typew.text}
          <span className="ml-1 inline-block w-[1ch] align-baseline text-white/90" style={{ opacity: typew.caretOn ? 1 : 0 }}>|</span>
        </h1>

        <div className="w-full max-w-[24rem] sm:max-w-[28rem] md:max-w-[32rem] 2xl:max-w-[42rem] transition-all duration-500">
          <div className="p-[1px] rounded-2xl bg-gradient-to-br from-white/70 via-white/20 to-transparent dark:from-zinc-700 dark:via-zinc-700/30 dark:to-transparent">
            <div className="rounded-2xl bg-gradient-to-br from-white/40 via-white/20 to-white/10 backdrop-blur-lg shadow-2xl border border-white/30 dark:from-zinc-900/40 dark:via-zinc-900/20 dark:to-zinc-900/10 dark:border-zinc-700/30">
              <div className="px-6 py-6 sm:px-7 sm:py-7 2xl:px-14 2xl:py-14">
                <div className="mb-5 2xl:mb-8 text-center">
                  <p className="text-sm 2xl:text-base text-zinc-600 dark:text-zinc-300">Inicia sesión para continuar.</p>
                </div>
                ...
                <form onSubmit={onSubmit} className="space-y-4">
                  <div>
                    <label htmlFor="login-usuario" className="block text-sm 2xl:text-base font-medium text-zinc-800 dark:text-zinc-200">Usuario</label>
                    <div className="mt-2 relative">
                      <span className="pointer-events-none absolute inset-y-0 left-3 2xl:left-4 flex items-center text-zinc-400 dark:text-zinc-500">
                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="2xl:w-5 2xl:h-5"><path d="M20 21a8 8 0 1 0-16 0" /><circle cx="12" cy="7" r="4" /></svg>
                      </span>
                      <input id="login-usuario" type="text" autoComplete="username" inputMode="email" autoCapitalize="none" autoCorrect="off"
                        className="block w-full rounded-xl border border-zinc-300/80 bg-white text-zinc-900 placeholder-zinc-400 focus:ring-2 focus:ring-[#7a24d8] focus:border-[#7a24d8] pl-10 2xl:pl-12 pr-3 py-2.5 2xl:py-3.5 2xl:text-lg dark:bg-zinc-900/60 dark:text-zinc-100 dark:placeholder-zinc-500 dark:border-zinc-700 transition-all font-medium"
                        placeholder="tu.usuario" {...register("usuario", { required: true })} />
                    </div>
                  </div>

                  <div>
                    <label htmlFor="login-password" className="block text-sm 2xl:text-base font-medium text-zinc-800 dark:text-zinc-200">Contraseña</label>
                    <div className="mt-2 relative">
                      <span className="pointer-events-none absolute inset-y-0 left-3 2xl:left-4 flex items-center text-zinc-400 dark:text-zinc-500">
                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="2xl:w-5 2xl:h-5"><rect x="3" y="11" width="18" height="11" rx="2" /><path d="M7 11V7a5 5 0 0 1 10 0v4" /></svg>
                      </span>
                      <input id="login-password" type={showPwd ? "text" : "password"} autoComplete="current-password" autoCapitalize="none" autoCorrect="off"
                        className="block w-full rounded-xl border border-zinc-300/80 bg-white text-zinc-900 placeholder-zinc-400 focus:ring-2 focus:ring-[#7a24d8] focus:border-[#7a24d8] pl-10 2xl:pl-12 pr-10 2xl:pr-12 py-2.5 2xl:py-3.5 2xl:text-lg dark:bg-zinc-900/60 dark:text-zinc-100 dark:placeholder-zinc-500 dark:border-zinc-700 transition-all font-medium"
                        placeholder="••••••••" {...register("contraseña", { required: true })} />
                      <button type="button" onClick={() => setShowPwd((v) => !v)} aria-label={showPwd ? "Ocultar contraseña" : "Mostrar contraseña"} className="absolute inset-y-0 right-2 2xl:right-3 inline-flex items-center px-2 text-zinc-500 hover:text-zinc-700 dark:text-zinc-400 dark:hover:text-zinc-200">
                        {showPwd ? (
                          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="2xl:w-6 2xl:h-6"><path d="M17.94 17.94A10.94 10.94 0 0 1 12 20c-5 0-9.27-3-11-8 1.04-2.71 2.98-4.94 5.41-6.31" /><path d="M1 1l22 22" /><path d="M9.88 9.88a3 3 0 1 0 4.24 4.24" /><path d="M6.1 6.1A10.94 10.94 0 0 1 12 4c5 0 9.27 3 11 8-.64 1.67-1.64 3.16-2.87 4.35" /></svg>
                        ) : (
                          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="2xl:w-6 2xl:h-6"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z" /><circle cx="12" cy="12" r="3" /></svg>
                        )}
                      </button>
                    </div>
                  </div>
                  ...
                  <button type="submit" disabled={submitting} aria-busy={submitting}
                    className={`w-full inline-flex justify-center items-center rounded-xl bg-gradient-to-r from-[#5115bc] to-[#E6007E] text-white px-5 py-3 2xl:py-5 2xl:text-xl font-semibold shadow-lg shadow-fuchsia-600/20 dark:shadow-none transition ${submitting ? "opacity-70 cursor-not-allowed" : "hover:brightness-110 active:scale-[.99]"}`}>
