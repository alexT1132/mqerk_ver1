...
function SectionBadge({ title, subtitle, onBack, total, onRefresh, isRefreshing }) {
  return (
    <div className="relative mx-auto max-w-8xl overflow-hidden rounded-2xl sm:rounded-3xl border-2 border-violet-200/60 bg-gradient-to-r from-violet-50/80 via-indigo-50/80 to-purple-50/80 p-4 sm:p-6 shadow-xl ring-2 ring-slate-100/50 mb-6 sm:mb-8">
      <div className="pointer-events-none absolute -left-10 -top-14 h-48 w-48 sm:h-56 sm:w-56 rounded-full bg-violet-200/50 blur-3xl" />
      <div className="pointer-events-none absolute -right-10 -bottom-14 h-48 w-48 sm:h-56 sm:w-56 rounded-full bg-indigo-200/50 blur-3xl" />

      <div className="relative z-10 flex items-center gap-3 sm:gap-4 md:gap-5">
        <button
          onClick={onBack}
          className="group/back flex h-9 w-9 sm:h-10 sm:w-10 items-center justify-center rounded-xl sm:rounded-2xl border-2 border-white/50 bg-white/30 backdrop-blur-md text-violet-700 hover:bg-white hover:border-white transition-all duration-200 shadow-sm active:scale-90 shrink-0"
          aria-label="Volver"
        >
          <ChevronLeft className="size-5 sm:size-6 transition-transform group-hover/back:-translate-x-0.5" strokeWidth={3} />
        </button>

        <div className="relative hidden xs:grid size-14 sm:size-16 place-items-center rounded-2xl sm:rounded-3xl bg-gradient-to-br from-violet-500 via-indigo-600 to-purple-600 text-white shadow-2xl ring-4 ring-white/50 shrink-0">
          <GraduationCap className="size-7 sm:size-8 text-white" />
          <div className="absolute -top-1 -right-1 inline-grid h-5 w-5 sm:h-6 sm:w-6 place-items-center rounded-full bg-gradient-to-br from-amber-400 to-orange-500 text-white ring-2 ring-white shadow-md">
            <span className="text-[10px] font-bold">★</span>
          </div>
        </div>

        <div className="flex flex-col min-w-0">
          <h2 className="text-2xl sm:text-3xl md:text-4xl 2xl:text-5xl font-extrabold tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-violet-600 via-indigo-600 to-purple-600 uppercase pb-1 sm:pb-2 pr-2">
            {title || "Módulos específicos"}
          </h2>
          <div className="mt-1.5 sm:mt-2 flex gap-2">
            <span className="h-1 sm:h-1.5 w-14 sm:w-20 rounded-full bg-gradient-to-r from-violet-500 to-indigo-500 shadow-sm" />
            <span className="h-1 sm:h-1.5 w-8 sm:w-12 rounded-full bg-gradient-to-r from-indigo-500 to-purple-500 shadow-sm" />
          </div>
        </div>

        <div className="hidden md:flex ml-auto items-center gap-2 sm:gap-3 shrink-0">
          {/* Botón de Refrescar */}
          {onRefresh && (
            <button
              onClick={onRefresh}
              disabled={isRefreshing}
              className="flex h-9 w-9 sm:h-10 sm:w-10 items-center justify-center rounded-xl sm:rounded-2xl border-2 border-white/50 bg-white/30 backdrop-blur-md text-violet-700 hover:bg-white hover:border-white transition-all duration-200 shadow-sm active:scale-90 disabled:opacity-50 disabled:cursor-not-allowed"
              title="Refrescar solicitudes"
            >
              <RefreshCw className={`size-4 sm:size-5 transition-transform ${isRefreshing ? 'animate-spin' : 'group-hover:rotate-180'}`} strokeWidth={2.5} />
            </button>
          )}

          {total > 0 && (
            <div className="flex items-center gap-2 sm:gap-3 rounded-xl sm:rounded-2xl bg-white/40 backdrop-blur-md px-3 sm:px-4 py-2 sm:py-3 border border-white/50 shadow-sm">
              <div className="flex h-8 w-8 sm:h-9 sm:w-9 items-center justify-center rounded-lg sm:rounded-xl bg-white text-violet-600 shadow-sm">
                <Layers className="size-4 sm:size-5" />
              </div>
              <div className="flex flex-col">
                <span className="text-base sm:text-lg font-black text-slate-800 leading-none">{total}</span>
                <span className="text-[9px] sm:text-[10px] font-bold text-slate-600 uppercase tracking-wider">Módulos</span>
              </div>
            </div>
          )}
        </div>
...
function AreaCard({ item, onOpenSolicitudes, badgeCount, to, state }) {
  ...
  return (
    <article
      className="group relative overflow-hidden rounded-xl sm:rounded-[1.5rem] p-4 sm:p-5 shadow-lg bg-white border-2 border-slate-900 hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 sm:hover:-translate-y-2 flex flex-col"
    >
      {/* Tint de fondo */}
      ...
      <h3 className="text-base sm:text-lg font-bold leading-tight text-slate-900 min-h-[2.25rem] sm:min-h-[2.5rem]">
        {title}
      </h3>
      <p className="mt-1.5 text-xs sm:text-sm text-slate-600 font-medium leading-relaxed mb-3 h-9 sm:h-10 overflow-hidden line-clamp-2">
        {desc}
      </p>
...
export default function ModulosEspecificos({
  ...
  return (
    <div className="min-h-screen relative">
      ...
      <section className="px-4 sm:px-6 lg:px-6 2xl:px-8 pt-4 sm:pt-6 pb-32 max-w-[min(1920px,100vw)] mx-auto">
        <SectionBadge
          title={header.title}
        ...
        {/* 5 por fila solo en 2xl (≥1536px); xl (ej. Huawei D15) mantiene 4 por fila (pantalla chica) */}
        <div className="mx-auto w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 2xl:grid-cols-5 gap-4 sm:gap-5">
          {items.map((item, i) => {
