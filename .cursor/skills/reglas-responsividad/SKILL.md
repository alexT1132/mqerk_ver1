...
### Modales "Generar con IA" (GeneradorIAModal, QuizIAModal)

> **Regla:** En pantallas chicas y medianas el modal no se toca; solo se amplía el ancho en **pantallas grandes (2xl)** y **extra grandes (≥1800px)**. No usar `max-w-[min(42rem,92vw)]` ni contenedores internos que limiten el ancho del contenido en este tipo de modal.

- **Base y hasta &lt; 2xl:** mantener `max-w-2xl` (672px), igual que el diseño inicial. No añadir breakpoints en `xl` para no disparar el ancho en laptops 13–14".
- **A partir de 2xl (≥1536px):** `2xl:max-w-4xl` (896px).
- **Pantallas ≥1800px:** `min-[1800px]:max-w-5xl` (1024px).
- **Altura (opcional):** `max-h-[75vh]` o `max-h-[90vh]`; en 2xl se puede usar `2xl:max-h-[85vh]` o `2xl:max-h-[92vh]`.
- **Body:** un único div con scroll (px/py y space-y). No envolver el contenido en un contenedor interno con `max-w-2xl mx-auto` para “evitar estiramiento”; el contenido debe llenar el modal.

**Clases del contenedor del modal (dialog):**

```jsx
className="... w-full max-w-2xl 2xl:max-w-4xl min-[1800px]:max-w-5xl max-h-[75vh] 2xl:max-h-[85vh] ..."
```

**Excepción — SimuladoresGen:** El modal "Generar simulador con IA" que está **inline** en `SimuladoresGen.jsx` (no usa el componente `GeneradorIAModal`) debe quedarse en `max-w-xl` **sin** añadir `2xl:max-w-4xl` ni `min-[1800px]:max-w-5xl`. No aplicar esta regla de ampliación en ese modal.

**Archivos afectados por la regla:** `client/src/components/Asesor/simGen/GeneradorIAModal.jsx`, `client/src/components/Asesor/simGen/QuizIAModal.jsx`.  
**Archivo excluido:** `client/src/components/Asesor/SimuladoresGen.jsx` (modal inline de IA).
...
- [ ] Si es modal "Generar con IA" (GeneradorIAModal/QuizIAModal): ¿base `max-w-2xl`, solo `2xl:max-w-4xl` y `min-[1800px]:max-w-5xl`? ¿No se aplica esta ampliación en el modal inline de SimuladoresGen?
