...
export const findAdvisorForGroup = async (group) => {
  if (!group) return null;
  // Buscar coincidencia exacta en grupo_asesor O dentro del JSON de grupos_asesor
  const sql = `
    SELECT usuario_id FROM asesor_perfiles 
    WHERE grupo_asesor = ? 
       OR JSON_SEARCH(grupos_asesor, 'one', ?) IS NOT NULL
    LIMIT 1
  `;
  const [rows] = await db.query(sql, [group, group]);
  return rows[0] || null;
};
