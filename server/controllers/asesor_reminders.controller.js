...
export const createForStudents = async (req, res) => {
  try {
    ...
    const { title, description, date, priority, studentIds, grupo } = req.body || {};
    if (!title || !date) return res.status(400).json({ message: 'title y date son obligatorios' });
    ...
    if (grupo) {
      ...
      const asesorNombre = `${asesorRows[0].nombres} ${asesorRows[0].apellidos}`.trim();
      ...
      const [rows] = await db.query(
        `SELECT e.id 
         ...
         WHERE e.id IN (${placeholders})
           ...
           AND e.asesor = ?
           AND sd.id IS NULL`,
        [...studentIds, ...gruposArr, asesorNombre]
      );
