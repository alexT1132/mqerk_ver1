...
import * as StudentResources from '../models/student_resources.model.js';
import path from 'path';
import fs from 'fs';
import { buildStaticUrl } from '../utils/url.js';

// Asegurar que la tabla exista al cargar el módulo
StudentResources.ensureTable().catch(err => {
  console.error('Error inicializando tabla student_resources:', err?.code || err?.message || err);
...
/**
 * Resolver ID de estudiante desde user ID
 */
async function resolveEstudianteId(userId) {
...
export const list = async (req, res) => {
  try {
    const resources = await StudentResources.listAll();
    ...
    console.error('student_resources list error:', e);
...
export const getById = async (req, res) => {
  try {
    const { id } = req.params;
    const resource = await StudentResources.getById(Number(id));
    ...
    console.error('student_resources getById error:', e);
    res.status(500).json({ message: 'Error interno' });
  }
};

/**
 * Crear nuevo recurso educativo (solo estudiantes)
 */
export const create = async (req, res) => {
  try {
    const estudianteId = await resolveEstudianteId(req.user.id);
    if (!estudianteId) return res.status(403).json({ message: 'Solo estudiantes' });
    ...
    const resource = await StudentResources.create(estudianteId, {
      title: title.trim(),
    ...
    console.error('student_resources create error:', e);
...
/**
 * Actualizar recurso educativo (solo el estudiante que lo subió)
 */
export const update = async (req, res) => {
  try {
    ...
    const ok = await StudentResources.update(Number(id), estudianteId, payload);
    if (!ok) return res.status(404).json({ message: 'Recurso no encontrado' });
    
    res.json({ ok: true });
  } catch (e) {
    console.error('student_resources update error:', e);
    res.status(500).json({ message: 'Error interno' });
  }
};

/**
 * Eliminar recurso educativo (solo el estudiante que lo subió)
 */
export const remove = async (req, res) => {
  try {
    ...
    const resource = await StudentResources.getById(Number(id));
    if (!resource || resource.estudiante_id !== estudianteId) {
      return res.status(404).json({ message: 'Recurso no encontrado' });
    }
    
    const ok = await StudentResources.remove(Number(id), estudianteId);
    if (!ok) return res.status(404).json({ message: 'Recurso no encontrado' });
    ...
    console.error('student_resources remove error:', e);
    res.status(500).json({ message: 'Error interno' });
  }
};
...
export const download = async (req, res) => {
  try {
    ...
    console.error('student_resources download error:', e);
    res.status(500).json({ message: 'Error interno' });
  }
};
