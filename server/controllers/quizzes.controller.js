...
// Crear quiz (actividad tipo 'quiz') + preguntas/opciones
export const createQuiz = async (req, res) => {
  try {
    ...
    const actId = await Actividades.createActividad({
      titulo,
      descripcion: descripcion || null,
      tipo: 'quiz',
      // Asociar al área/materia si llega (para filtrar por módulo/área en el alumno)
      id_area: id_area || null,
      materia: materia || null,
      puntos_max: 100,
      fecha_limite: fecha_limite || null,
      max_intentos: max_intentos || null,
      requiere_revision: 0,
      activo: 1,
      ...
      // grupos: asignación directa a grupos de estudiantes
      grupos: req.body.grupos || null,
      // guardar creador si hay usuario autenticado
      creado_por: req.user?.id || null,
    });
    // Insertar preguntas si llegan (si existen tablas de preguntas/opciones)
    if (Array.isArray(preguntas) && preguntas.length) {
      try {
        let orden = 1;
        for (const q of preguntas) {
          const row = await QQ.createPregunta({
            id_quiz: actId,
            orden: q.orden ?? orden,
            enunciado: q.text || q.enunciado || '',
            tipo: q.type === 'tf' ? 'verdadero_falso' : (q.type === 'short' ? 'respuesta_corta' : 'opcion_multiple'),
            puntos: q.points || 1,
            activa: 1,
          });
          // opciones
          if (Array.isArray(q.options) && q.options.length) {
            ...
            for (const opt of q.options) {
              await QQ.createOpcion({ id_pregunta: row.id, texto: opt.text || '', es_correcta: !!opt.correct, orden: o++ });
            }
          } else if (q.type === 'tf') {
            await QQ.createOpcion({ id_pregunta: row.id, texto: 'Verdadero', es_correcta: q.answer === 'true', orden: 1 });
            await QQ.createOpcion({ id_pregunta: row.id, texto: 'Falso', es_correcta: q.answer === 'false', orden: 2 });
          } else if (q.type === 'short') {
            // Para respuesta corta, opcionalmente se puede guardar como opción textual de referencia
            if (q.answer) await QQ.createOpcion({ id_pregunta: row.id, texto: q.answer, es_correcta: 1, orden: 1 });
          }
          orden++;
        }
      } catch (e) {
        if (e?.code === 'ER_NO_SUCH_TABLE' || e?.errno === 1146) {
          console.warn('createQuiz: tablas de preguntas/opciones no existen; se creó solo la actividad base', e?.message || e);
        } else {
          throw e;
        }
      }
    }
    const created = await Actividades.getActividad(actId);

    // Notificar a estudiantes por grupos si se especificaron
    try {
      const grupos = created?.grupos || req.body.grupos;
      if (Array.isArray(grupos) && grupos.length) {
        const placeholders = grupos.map(()=>'?').join(',');
        const [rows] = await db.query(`SELECT id FROM estudiantes WHERE grupo IN (${placeholders})`, grupos);
        if (rows.length) {
          const list = rows.map(r => ({
            student_id: r.id,
            type: 'assignment',
            title: 'Nuevo quiz asignado',
            message: created.titulo ? `Se te asignó: ${created.titulo}` : 'Tienes un nuevo quiz',
          ...
          rows.forEach((r, idx) => {
            broadcastStudent(r.id, { type:'notification', payload: { kind:'assignment', actividad_id:actId, title: 'Nuevo quiz', message: created.titulo, notif_id: idMap[idx] } });
          });
        }
      }
    } catch (e) { console.error('notif createQuiz', e); }
    res.status(201).json({ data: created });
  } catch(e){ console.error('createQuiz', e); res.status(500).json({ message:'Error interno'}); }
};

// Eliminar quiz (actividad tipo 'quiz') y sus dependencias
export const deleteQuiz = async (req, res) => {
  ...
  try {
    ...
    const [existsRows] = await conn.query('SELECT id FROM actividades WHERE id = ? AND tipo = "quiz" LIMIT 1', [id]);
    ...
    const [del] = await conn.query('DELETE FROM actividades WHERE id = ? AND tipo = "quiz"', [id]);
...
export const updateQuiz = async (req, res) => {
  ...
  try {
    ...
    const [existsRows] = await conn.query('SELECT id, titulo, publicado, grupos FROM actividades WHERE id = ? AND tipo = "quiz" LIMIT 1', [id]);
    if (!existsRows.length) {
      await conn.rollback();
      return res.status(404).json({ message: 'Quiz no encontrado' });
    }
    ...
    await Quizzes.updateQuizMeta(id, {
      titulo, descripcion, materia, id_area: id_area ?? null, fecha_limite: fecha_limite ?? null,
      max_intentos: max_intentos ?? null, publicado: publico ? 1 : 0,
      shuffle_questions: !!shuffle_questions, time_limit_min: time_limit_min ?? null,
      passing_score: passing_score ?? null, activo: activo ?? 1
    });

    // Si se envían grupos, actualizarlos mediante modelo de actividades (maneja JSON)
    if (req.body.grupos !== undefined) {
      ...
      try { await Actividades.updateActividad(id, { grupos: gruposVal }); } catch(e) { console.error('updateQuiz:set grupos', e); }
    }
    ...
    // Si se publicó (transición 0 -> 1), notificar a grupos
    try {
        ...
        if (Array.isArray(grupos) && grupos.length) {
          ...
          if (rows.length) {
            const list = rows.map(r => ({
              ...
              type: 'assignment',
              title: 'Nuevo quiz asignado',
              message: (titulo || existing.titulo) ? `Se te asignó: ${titulo || existing.titulo}` : 'Tienes un nuevo quiz',
            ...
            rows.forEach((r, idx) => {
              broadcastStudent(r.id, { type:'notification', payload: { kind:'assignment', actividad_id:Number(id), title: 'Nuevo quiz', message: titulo || existing.titulo, notif_id: idMap[idx] } });
            });
          }
        }
      }
